...
import 'package:dbminer_animal/helper/api_helper.dart';
import 'package:dbminer_animal/helper/imagelist.dart';
...
class DBhelper {
  ...
  Future<Database?> initDb() async {
    ...
    db = await openDatabase(databasePath, version: 1, onCreate: (Database db, version) async {
...
//   }
  Future<void> insertData() async {
    ...
    while (i < animaListData.length) {
      ...
      String query = "INSERT INTO animal(id, name,color,skin_type,top_speed,weight,length,image) VALUES(?,?,?,?,?,?,?,?)";
      List arg = [
        null,
        '${post.name}',
        '${post.color}',
